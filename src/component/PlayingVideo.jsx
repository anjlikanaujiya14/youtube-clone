...
function PlayingVideo() {
    ...
    const fetchVideoDetails = () => {
        fetchData(`video/details/?id=${id}`).then((res) => {
            console.log('Video Details Response:', res); // Log the entire response
            if (res.author) {
                console.log('Author Data:', res.author); // Log author data
                console.log('Author Avatar URL:', res.author.avatar); // Log avatar URL
            }
            setVideo(res);
        }).catch(error => {
    ...
    const fetchRelatedVideo = () => {
        fetchData(`video/related-contents/?id=${id}`).then((res) => {
            console.log('Related Video Response:', res); // Log response
            setRelatedVideo(res);
        }).catch(error => {
        ...
        <div className="flex justify-center flex-row h-[calc(100%-56px)] mt-16">
            <div className="w-full max-w-[1580px] flex flex-col lg:flex-row">
                <div className="flex flex-col lg:w-[calc(100%-350px)] xl:w-[100%-400px] px-4 py-3 lg:py-6">
                    <div className="h-[200px] md:h-[500px] ml-[-16px] mr-[-16px] lg:ml-10 lg:mr-20">
                        <ReactPlayer
                    ...
                    <div className="font-bold text-sm md:text-xl mt-4 line-clamp-2 lg:ml-10">
                        {video?.title}
                    </div>
                    <div className="flex justify-between flex-col md:flex-row mt-4">
                            ...
                            <div className="flex items-start">
                                <div className="flex h-11 w-11 rounded-full overflow-hidden">
                                    <img
                                        ...
                                        alt={video?.author?.title || "Author Avatar"} // Alt text
                                    />
                                </div>
                            </div>
                        ...
                        <div className="flex mt-4 md:mt-0">
                            <div className="flex items-center justify-center h-11 px-6 rounded-3xl bg-white/[0.15]">
                                ...
                                {`${abbreviateNumber(video?.stats?.likes, 2) || 0} Likes`}
                            </div>
                            <div className="flex items-center justify-center h-11 px-6 rounded-3xl bg-white/[0.15] ml-4">
                                {`${abbreviateNumber(video?.stats?.views, 2) || 0} Views`}
                            </div>
                        </div>
                    </div>
                    <div className="p-4 bg-gray-100 rounded-xl mt-4 text-sm lg:ml-7">
                        {video?.description}
                    </div>
                    <div className="flex gap-x-6 font-semibold rounded-xl mt-4 text-xl lg:ml-10">
